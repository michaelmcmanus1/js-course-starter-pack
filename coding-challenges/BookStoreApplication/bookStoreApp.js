...
var checkStore = function(booktitle){
	for (var i = 0; i < BookStore.length; i++) {
		...
		if (BookStore[i]["title"] === booktitle) {
			var response = prompt("We found your book! " + booktitle + ", by " + BookStore[i]["author"] + ". It costs " + BookStore[i]["price"] + ". \n Would you like to add it to your cart?");
	...
	if(noBook === false) {
			alert("Your book was not found!");
		}
};

// Declare a new method called deleteBook that takes a string argument
// Iterate through the cart array in Susan's object
// If the book exists, confirm they want to delete it from there.
